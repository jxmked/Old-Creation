...
var tsc_hex = {};
var temp_lever = false; //Except passing to for loop, I am running out of ideas how to 
...
function TSC(str, val){
	...
	switch((val) ? val : $("#TSC").attr("data")){
		...
		case "1": //TSC Translate Character to something
			//Since I translated the string to hex first...
			//We need to translate first the special char first too... before the process
			var tsc = getTSC();
			
			if(!temp_lever){
				//This thing may Cause slower system but it worked!!!!
				for(var index in tsc){
					var a = toHex([index]).join("");
					tsc_hex[a] = toHex([tsc[index]]).join("");
				}
				temp_lever = true;
			}

			return str.map(function(hex){
				return (typeof tsc_hex[hex] == "undefined") ? hex : tsc_hex[hex];
			});
			break;
	} //End switch
...
function ULF(str, val){
	...
	const Low = function(val){
		...
		return val.charAt(0).toLowerCase() + val.substr(1).toUpperCase();
	}; //End Low function
